...
@Configuration
@MapperScan(basePackages = {"com.syscom.fep.mybatis.ems.mapper", "com.syscom.fep.mybatis.ems.ext.mapper"}, annotationClass = Resource.class, sqlSessionTemplateRef = DataSourceEmsConstant.BEAN_NAME_SQL_SESSION_TEMPLATE)
@RefreshScope
public class DataSourceEmsConfiguration implements DataSourceEmsConstant {
    // @Value("${spring.fep.mybatis.ems.interceptor.connection.detected.enable:false}")
    ...
    public DataSource dataSource(@Qualifier(BEAN_NAME_HIKARI_CONFIG) HikariConfig hikariConfig) throws Exception {
        ...
        File acctFile = pkiConfig.getPkiAcctFile(FEPDBName.EMSDB);
        LogHelperFactory.getGeneralLogger().info(" ems acdu Path : "+acctFile);
        File sscodeFile = pkiConfig.getPkiSscodeFile(FEPDBName.EMSDB);
        LogHelperFactory.getGeneralLogger().info(" ems acdp Path : "+sscodeFile);
        try (InputStream acctIn = IOUtil.openInputStream(acctFile.getPath());
            ...
            LogHelperFactory.getGeneralLogger().info(" ems username  : "+acctProp.getProperty(CONFIGURATION_PROPERTIES_USERNAME));
            LogHelperFactory.getGeneralLogger().info(" ems password  : ***"+Jasypt.decrypt(sscodeProp.getProperty(CONFIGURATION_PROPERTIES_PASSWORD)));
...
//        DynamicDataSource dynamicDataSource = new DynamicDataSource(FEPDBName.EMSDB.name());
...
//    }
    ...
    public SqlSessionFactory testSqlSessionFactory(@Qualifier(BEAN_NAME_DATASOURCE) DataSource dataSource) throws Exception {
        ...
        bean.setMapperLocations(
                ArrayUtils.addAll(
                        new PathMatchingResourcePatternResolver().getResources("classpath*:com/syscom/fep/mybatis/ems/mapper/xml/*.xml"),
                        new PathMatchingResourcePatternResolver().getResources("classpath*:com/syscom/fep/mybatis/ems/ext/mapper/xml/*.xml")));
        bean.setConfigLocation(new PathMatchingResourcePatternResolver().getResource("classpath:mybatis-config.xml"));// 增加攔截器
        List<Interceptor> interceptorList = new ArrayList<Interceptor>();
//        if (dataSource instanceof DynamicDataSource && this.connectionDetectedEnable) {
//            EmsConnectionDetectedInterceptor emsConnectionDetectedInterceptor = new EmsConnectionDetectedInterceptor();
//            interceptorList.add(emsConnectionDetectedInterceptor);
//        }
    ...
    public void print() {
        printConstant();
        Field[] fields = DataSourceEmsConfiguration.class.getDeclaredFields();

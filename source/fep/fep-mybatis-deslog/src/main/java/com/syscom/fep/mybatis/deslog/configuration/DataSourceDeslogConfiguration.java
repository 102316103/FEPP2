package com.syscom.fep.mybatis.deslog.configuration;
...
@Configuration
@MapperScan(basePackages = {"com.syscom.fep.mybatis.deslog.mapper", "com.syscom.fep.mybatis.deslog.ext.mapper"}, annotationClass = Resource.class, sqlSessionTemplateRef = DataSourceDeslogConstant.BEAN_NAME_SQL_SESSION_TEMPLATE)
@RefreshScope
public class DataSourceDeslogConfiguration implements DataSourceDeslogConstant {
    // @Value("${spring.fep.mybatis.deslog.interceptor.connection.detected.enable:false}")
    ...
    public DataSource dataSource(@Qualifier(BEAN_NAME_HIKARI_CONFIG) HikariConfig hikariConfig) throws Exception {
        ...
        File acctFile = pkiConfig.getPkiAcctFile(FEPDBName.DESLOGDB);
        LogHelperFactory.getGeneralLogger().info(" deslog acdu Path : "+acctFile);
        File sscodeFile = pkiConfig.getPkiSscodeFile(FEPDBName.DESLOGDB);
        LogHelperFactory.getGeneralLogger().info(" deslog acdp Path  : "+sscodeFile);
        try (InputStream acctIn = IOUtil.openInputStream(acctFile.getPath());
            ...
            LogHelperFactory.getGeneralLogger().info(" deslog username  : "+acctProp.getProperty(CONFIGURATION_PROPERTIES_USERNAME));
            LogHelperFactory.getGeneralLogger().info(" deslog password  : ***"+Jasypt.decrypt(sscodeProp.getProperty(CONFIGURATION_PROPERTIES_PASSWORD)));
...
//        DynamicDataSource dynamicDataSource = new DynamicDataSource(FEPDBName.DESLOGDB.name());
...
//    }
    ...
    public SqlSessionFactory testSqlSessionFactory(@Qualifier(BEAN_NAME_DATASOURCE) DataSource dataSource) throws Exception {
        ...
        bean.setMapperLocations(
                ArrayUtils.addAll(
                        new PathMatchingResourcePatternResolver().getResources("classpath*:com/syscom/fep/mybatis/deslog/mapper/xml/*.xml"),
                        new PathMatchingResourcePatternResolver().getResources("classpath*:com/syscom/fep/mybatis/deslog/ext/mapper/xml/*.xml")));
        bean.setConfigLocation(new PathMatchingResourcePatternResolver().getResource("classpath:mybatis-config.xml"));
        // 增加攔截器
        List<Interceptor> interceptorList = new ArrayList<Interceptor>();
//        if (dataSource instanceof DynamicDataSource && this.connectionDetectedEnable) {
//            DeslogConnectionDetectedInterceptor deslogConnectionDetectedInterceptor = new DeslogConnectionDetectedInterceptor();
//            interceptorList.add(deslogConnectionDetectedInterceptor);
//        }
    ...
    public void print() {
        printConstant();
        Field[] fields = DataSourceDeslogConfiguration.class.getDeclaredFields();

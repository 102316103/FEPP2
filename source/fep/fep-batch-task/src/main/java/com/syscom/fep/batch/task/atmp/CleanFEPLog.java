 ...
 * 清除次系統日的FEPLOG
 */
public class CleanFEPLog extends FEPBase implements Task {
	private boolean batchResult = false;
	private String _batchLogPath = StringUtils.EMPTY;
	private BatchJobLibrary job = null;
	private FeplogExtMapper feplogextmapper = SpringBeanFactoryUtil.getBean(FeplogExtMapper.class);
	...
	private boolean mainProcess() {
		...
		try {
			...
			SimpleDateFormat formatDayInWeek = new SimpleDateFormat("u"); // 僅取得星期，用於feplog patation

			// 若沒輸入參數則使用系統日期
			if (StringUtils.isBlank(businessDate) && StringUtils.length(businessDate) != 8) {
				businessDate = formatDay.format(new Date());
			}
			job.writeLog(String.format("清除日期:%s的次系統日的FEPLog資料", businessDate));
...
//			Date endDate = endCalendar.getTime();
			...
			int count = this.feplogextmapper.TruncateByLogDate(dayInWeek);
//			int count = this.feplogextmapper.deleteByLogDate(dayInWeek, startDate, endDate);
			job.writeLog("delete FEPLOG By LogDate count : " + count);

			job.writeLog("CleanFEPLog 執行正常!");
		} catch (Exception ex) {
			job.writeLog("CleanFEPLog執行失敗");
	...
	private void DisplayUsage() {
		...
		System.out.println("  CleanFEPLog Options");
		System.out.println();
		...
		System.out.println("  execute system day        CleanFEPLog");
		System.out.println("  execute specifies day     CleanFEPLog /TBSDY:20100101");
	}
}

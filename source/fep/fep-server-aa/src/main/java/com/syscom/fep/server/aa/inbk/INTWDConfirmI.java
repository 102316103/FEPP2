...
public class INTWDConfirmI extends INBKAABase {
     ...
     * @throws Exception
     */
    public INTWDConfirmI(FISCData txnData) throws Exception {
        super(txnData);
    }
...
//		''' <remarks></remarks>
    private FEPReturnCode updateTxData() {
        ...
        try {
            ...
            int iRes = 0;
            defintltxn.setIntltxnTxDate(getFiscBusiness().getFeptxn().getFeptxnTxDate());
            defintltxn.setIntltxnEjfno(getFiscBusiness().getFeptxn().getFeptxnEjfno());
            defintltxn = dbINTLTXN.selectByPrimaryKey(defintltxn.getIntltxnTxDate(), defintltxn.getIntltxnEjfno());
            if (defintltxn != null) {
                defintltxn.setIntltxnConRc(getFiscBusiness().getFeptxn().getFeptxnConRc());
                ...
                if (iRes <= 0) {
                    transactionManager.rollback(txStatus);
                    _rtnCode = IOReturnCode.UpdateFail;
                    return _rtnCode;
                }
            }

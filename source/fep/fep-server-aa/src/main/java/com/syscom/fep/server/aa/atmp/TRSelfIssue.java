...
public class TRSelfIssue extends ATMPAABase {
	...
	private void checkBusinessRule() throws Exception {
        ...
        if("TM".equals(TaCode)) {
			...
			this.logContext.setMessage("Enter into MobileAdapter");
			logMessage(this.logContext);
			try {
				...
				// 2.手機門號
				request.setMobilePhone(feptxn.getFeptxnTelephone());
				// 3.bankCode 不用給值
				request.setBankCode("");
				...
				this.logContext.setMessage(StringUtils.join("into MobileAdapter rtnCode ", rtnCode.toString()));
				logMessage(this.logContext);
			} catch (Exception e) {
	...
	private String response() {
		...
		try {
			...
			if (rtnCode != FEPReturnCode.Normal) {
				...
				if (rtnCode != FEPReturnCode.Normal) { //FEP檢核錯誤或IMS錯誤
				...
				if (rtnCode != FEPReturnCode.Normal) {
					...
					atm_faa_cc1b1pn.setPRCCODE(feptxn.getFeptxnCbsRc()); //主機三碼表示交易成功
				}

		...
		try {
			...
			if (uiPCODE.equals("2280")) {
				...
				if (Double.parseDouble(LocalDateTime.now().format(DateTimeFormatter.ofPattern("HHmm"))) >= 1500
						&& Double.parseDouble(LocalDateTime.now().format(DateTimeFormatter.ofPattern("HHmm"))) <= 1600
			...
			if (uiPCODE.equals("2280")) {
				/* 當日 PENDING 交易 */
				wk_SYS_DATETIME = LocalDateTime.now().plusMinutes(-5)
						.format(DateTimeFormatter.ofPattern("yyyyMMddHHmmss"));
				// 讀取 FEPTXN /*檔名SEQ為 wk_DATE [7:2]*/
				String tbsdy = wk_DATE.substring(6, 8); // 檔名SEQ為 FEPTXN_TBSDY_FISC[7:2]
				...
				dsFeptxn = feptxnDao.getFEPTXNFor2280(wk_SYS_DATETIME, getFiscBusiness().getFeptxn().getFeptxnTxDate(), getFiscBusiness().getFeptxn().getFeptxnBkno(),

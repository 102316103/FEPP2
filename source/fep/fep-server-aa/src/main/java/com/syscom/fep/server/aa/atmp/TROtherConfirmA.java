...
public class TROtherConfirmA extends INBKAABase {
    ...
    private void checkBusinessRule() throws Exception {
		...
		if (StringUtils.isBlank(ATM_TITA_PICCMACD)) {
            ...
            logMessage(this.logContext);
			rtnCode2 = new ENCHelper(getATMtxData()).checkAtmMac(feptxn.getFeptxnAtmno(), wkMAC, ATMMAC);
    ...
    private void updateTxData() {
        ...
        if (DbHelper.toBoolean(getATMtxData().getMsgCtl().getMsgctlAtm2way())) {
            if ("NN".equals(getATMRequest().getSTATUS().substring(0,2))) {
    ...
    private void sendtoCBS() {
        try {
            if(feptxn.getFeptxnPcode().equals("2521") || feptxn.getFeptxnPcode().equals("2522")) {
                String AATxTYPE = "";
                String AATxRs = "N";

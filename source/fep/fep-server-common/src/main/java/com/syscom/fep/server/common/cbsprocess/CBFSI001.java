...
import com.syscom.fep.vo.text.ims.CB_FS_I001;
import com.syscom.fep.vo.text.ims.CB_FS_O001;
 ...
 * 
 */
public class CBFSI001 extends ACBSAction {

	public CBFSI001(MessageBase txData) {
		super(txData, new CB_FS_O001());
	}
	...
	public FEPReturnCode getCbsTita(String txtype) throws Exception {
		/* TITA 請參考合庫主機電文規格(CB_FS_I001)*/
		/* ATM.REQ 請參考 ATM_GeneralTrans */
		CB_FS_I001 cbstita = new CB_FS_I001();
		...
		cbstita.setICMEMO(getAtmData().getTxRequestMessage().substring(446, 506));
		//TAC(LL+B(V)，ByteArray 型態B(240))
		cbstita.setTXNICCTAC(getAtmData().getTxRequestMessage().substring(758, 778));
		cbstita.setFROMACT(feptxn.getFeptxnMajorActno()); // 卡片帳號
		//FS：指靜脈申請、RS：指靜脈解鎖
		cbstita.setTR_SPECIAL_FLAG(atmReq.getFADATA().substring(0, 2));//FADATA[1:2]
		cbstita.setCUSTID(feptxn.getFeptxnIdno()); //身分證ID 
		cbstita.setBIRDAY(atmReq.getSPECIALDATA().substring(46, 53));//客戶出生日期 SPECIALDATA[47:7]  
		cbstita.setPITELNO(atmReq.getSPECIALDATA().substring(0, 10));//手機門號 SPECIALDATA[1:10]  
	...
	public FEPReturnCode processCbsTota(String cbsTota, String type) throws Exception {
		/* 電文內容格式請參照TOTA電文格式(CB_FS_O001) */
		CB_FS_O001 tota = new CB_FS_O001();
	 ...
	 * @throws Exception
	 */
	private FEPReturnCode updateFEPTxn(CB_FS_O001 cbsTota) throws Exception {

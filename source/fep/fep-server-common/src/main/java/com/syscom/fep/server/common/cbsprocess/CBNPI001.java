...
import com.syscom.fep.vo.text.ims.CB_NP_I001;
import com.syscom.fep.vo.text.ims.CB_NP_O001;
import org.apache.commons.lang3.StringUtils;

import java.util.Calendar;

/**
 * 組送CBS主機網銀代號密碼重設Request交易電文
 * 
 * @author Joseph
 *
 */

public class CBNPI001 extends ACBSAction {

	public CBNPI001(MessageBase txData) {
		super(txData, new CB_NP_O001());
	}
...
//	private SmsmsgExtMapper smsmsgExtMapper = SpringBeanFactoryUtil.getBean(SmsmsgExtMapper.class);
	...
	public FEPReturnCode getCbsTita(String txType) throws Exception {
		/* TITA 請參考合庫主機電文規格(CB_NP_I001) */
		// Header
		CB_NP_I001 cbstita = new CB_NP_I001();
		...
		cbstita.setBIRDAY(atmReq.getSPECIALDATA().substring(46,53)); //出生日期
		cbstita.setPINPFUN(atmReq.getSPECIALDATA().substring(0,1)); //重設項目
		cbstita.setPIEBACLL(atmReq.getSPECIALDATA().substring(1,3)); //重設驗證碼長度
		cbstita.setPIEBNPAC(atmReq.getSPECIALDATA().substring(3,16)); //認證碼
	...
	public FEPReturnCode processCbsTota(String cbsTota, String type) throws Exception {
		/* 電文內容格式請參照TOTA電文格式(CB_NP_O001) */
		/* 拆解主機回應電文 */
		CB_NP_O001 tota = new CB_NP_O001();
	 ...
	 * @throws Exception
	 */
	private FEPReturnCode updateFEPTxn(CB_NP_O001 cbsTota) throws Exception {

...
import com.syscom.fep.vo.text.ims.IB_IQ_I001;
...
/**
 * 組送CBS 主機原存餘額查詢Request交易電文
 *
 * @author Joseph
 */

public class IBIQI001 extends ACBSAction {

    public IBIQI001(MessageBase txData) {
        super(txData, new IB_IQ_I001());
    }
...
//    private SmsmsgExtMapper smsmsgExtMapper = SpringBeanFactoryUtil.getBean(SmsmsgExtMapper.class);
    ...
    public FEPReturnCode getCbsTita(String txType) throws Exception {
        //組CBS 原存交易Request電文, 電文內容格式請參照: D_I0_合庫FEP_主機電文規格-原存行餘額查詢交易V1.01(1111020)
        IB_IQ_I001 cbsTita = new IB_IQ_I001();
        // HEADER
        ...
        // 20221007 Add for 跨國餘額查詢
        if ("2505".equals(feptxn.getFeptxnPcode())) {
            cbsTita.setBIT35_JRNATION(this.getFiscData().getIctlTxn().getIctltxnAcqCntry()); // 收單國別碼
            cbsTita.setBIT35_JRCCD(this.getFiscData().getIctlTxn().getIctltxnTxCur()); // 收單幣別碼
            cbsTita.setBIT35_JTAIFEE(this.getFiscData().getIctlTxn().getIctltxnTwdFee()); // 台方手續費
            cbsTita.setBIT35_WAMTDISP(this.getFiscData().getIctlTxn().getIctltxnTxAmt()); // 原幣提領金額/消費金額
        } else {
            cbsTita.setBIT35_JRNATION(StringUtils.repeat(" ", 3));
            cbsTita.setBIT35_JRCCD(StringUtils.repeat(" ", 3));
            cbsTita.setBIT35_JTAIFEE(BigDecimal.ZERO);
            cbsTita.setBIT35_WAMTDISP(BigDecimal.ZERO);
        }
        cbsTita.setDRVS(StringUtils.repeat(" ", 353)); // 保留欄位
    ...
    public FEPReturnCode processCbsTota(String cbsTota, String type) throws Exception {
        /* 電文內容格式請參照TOTA電文格式(IB_IQ_O001) */
        /* 拆解主機回應電文 */
        IB_IQ_O001 tota = new IB_IQ_O001();
     ...
     * @throws Exception
     */
    private FEPReturnCode updateFEPTxn(IB_IQ_O001 cbsTota, String type) throws Exception {
        ...
        if (StringUtils.isNotBlank(cbsTota.getAVAILABLE_BALANCE())) {
            feptxn.setFeptxnBala(new BigDecimal(cbsTota.getAVAILABLE_BALANCE())); /* 帳戶餘額 */
        }
    ...
    private void insertSMSMSG(IB_IQ_O001 cbsTota) throws ParseException {

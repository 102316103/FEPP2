...
import com.syscom.fep.vo.text.ims.CB_FC5_I001;
import com.syscom.fep.vo.text.ims.CB_FC5_O001;
...
public class CBFC5I001 extends ACBSAction {

	public CBFC5I001(MessageBase txData) {
		super(txData, new CB_FC5_O001());
	}
	...
	public FEPReturnCode getCbsTita(String txType) throws Exception {
		/* TITA 請參考合庫主機電文規格(CB_FC5_I001) */
		/* ATM.REQ 請參考 ATM_GeneralTrans */
		// Header
		CB_FC5_I001 cbstita = new CB_FC5_I001();
		...
		cbstita.setCUSTTYPE(StringUtils.substring(atmReq.getTADATA(), 10, 11)); // 身分別代號
	...
	public FEPReturnCode processCbsTota(String cbsTota, String type) throws Exception {
		/* 電文內容格式請參照TOTA電文格式(CB_FC5_O001) */
		/* 拆解主機回應電文 */
		CB_FC5_O001 tota = new CB_FC5_O001();
	 ...
	 * @throws Exception
	 */
	private FEPReturnCode updateFEPTxn(CB_FC5_O001 cbsTota) throws Exception {
		...
		if (!cbsTota.getIMSRC_TCB().equals("000")) {
			...
			//外幣匯率
			if(feptxn.getFeptxnTxCur().equals("USD") && StringUtils.isNotBlank(cbsTota.getFR5DATA())){
				feptxn.setFeptxnExrate(new BigDecimal(cbsTota.getFR5DATA().substring(9,17)));
			}
			if(feptxn.getFeptxnTxCur().equals("JPY") && StringUtils.isNotBlank(cbsTota.getFR5DATA())){
				feptxn.setFeptxnExrate(new BigDecimal(cbsTota.getFR5DATA().substring(19,27)));
			}
			/* 正常回應 */

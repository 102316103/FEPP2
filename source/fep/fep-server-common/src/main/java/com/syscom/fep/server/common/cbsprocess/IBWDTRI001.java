...
import com.syscom.fep.vo.text.ims.IB_WD_TR_I001;
...
/**
 * 組送CBS 主機原存跨國晶片提款Request交易電文
 *
 * @author Joseph
 *
 */

public class IBWDTRI001 extends ACBSAction {

	public IBWDTRI001(MessageBase txData) {
		super(txData, new IB_WD_TR_I001());
	}
...
//	private SmsmsgExtMapper smsmsgExtMapper = SpringBeanFactoryUtil.getBean(SmsmsgExtMapper.class);
	...
	public FEPReturnCode getCbsTita(String txType) throws Exception {
		...
		IB_WD_TR_I001 cbsTita = new IB_WD_TR_I001();
		// HEADER
		...
		if(FISCPCode.PCode2572.getValueStr().equals(feptxn.getFeptxnPcode())){
			cbsTita.setJ2_ORG_STAN(inbkReq.getOriStan()); // 原交易序號
			cbsTita.setJ2_Reason_Code(inbkReq.getRsCode()); // 消費扣款沖正理由
		}else{
			cbsTita.setJ2_ORG_STAN(StringUtils.repeat(" ",10)); // 原交易序號
			cbsTita.setJ2_Reason_Code(StringUtils.repeat(" ",2)); // 消費扣款沖正理由
		}
		...
		cbsTita.setBIT35_JTAIFEE(ictltxn.getIctltxnTwdFee()); // 台方手續費
		cbsTita.setBIT35_WAMTDISP(ictltxn.getIctltxnTxAmt()); // 原幣提領金額／消費金額
		cbsTita.setDRVS(StringUtils.repeat(" ", 341)); // 保留欄位
	...
	public FEPReturnCode processCbsTota(String cbsTota, String type) throws Exception {
		/* 電文內容格式請參照TOTA電文格式(IB_WD_TR_O001) */
		/* 拆解主機回應電文 */
		IB_WD_TR_O001 tota = new IB_WD_TR_O001();
	 ...
	 * @throws Exception
	 */
	private FEPReturnCode updateFEPTxn(IB_WD_TR_O001 cbsTota, String type) throws Exception {
	...
	private void insertSMSMSG(IB_WD_TR_O001 cbsTota) throws ParseException {

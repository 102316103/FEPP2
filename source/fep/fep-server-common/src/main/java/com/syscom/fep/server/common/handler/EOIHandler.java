...
public class EOIHandler extends HandlerBase {
    ...
    public String dispatch(FEPChannel channel, String data) {
        ...
        String res = "";
        RCV_NB_GeneralTrans_RQ_Body_MsgRq_Header atmReqheader = null;
        ...
        try {
            ...
            //根據testLEIssueRequestA()，位置在195，不是spec的188
            String msgidPre = data.substring(195, 197);
            String txntype = "";

            // 1. 傳入值    Channel  varchar(10)  通道別
            if ("LE".equals(msgidPre) || "LF".equals(msgidPre)) {
            ...
            String timeStart = "";
            /*ID+ACCT 全繳不用檢核*/
            if (FEPChannel.EOI.equals(channel.name().substring(0, 1))) {
                if (StringUtils.isNotBlank(atmReqbody.getINDATE())
                    ...
                    if (diffseconds > 90 && !FEPChannel.EOI.equals(channel)) {
                        ...
                        if ("IQ".equals(txntype) || "RV".equals(txntype)) {
                            ...
                            if ("RQ".equals(txntype)) {
                                nbData.getAtmtxObject().getRequest().setPICCBI55("202020202020202020202020202020");/*ICMARK*/

package com.syscom.fep.vo.text.unisys.mo.request;

import com.syscom.fep.frmcommon.annotation.Field;
import com.syscom.fep.frmcommon.util.StringUtil;
import com.syscom.fep.vo.text.unisys.mo.MOGeneral;
import com.syscom.fep.vo.text.unisys.mo.MOGeneralRequest;
import com.syscom.fep.vo.text.unisys.mo.MOUnisysTextBase;
import org.apache.commons.lang3.StringUtils;

public class MOJ10205Request extends MOUnisysTextBase {
    ...
    //OORATE
    @Field(length = 20, optional = true)
    private String _OORATE = "";

    //OCRATE
    @Field(length = 20, optional = true)
    private String _OCRATE = "";

    //OPBAMT
    @Field(length = 26, optional = true)
    private String _OPBAMT = "";
    ...
    //
    @Field(length = 520, optional = true)
    ...
    /**
     ...
     <remark></remark>
     */
    public String getOORATE() {
        return _OORATE;
    }
    public void setOORATE(String value) {
        _OORATE = value;
    }
    ...
    public void setOCRATE(String value) {
        _OCRATE = value;
    }
    ...
    public void setOPBAMT(String value) {
        _OPBAMT = value;
    }
    ...
    /**
     ...
     <remark></remark>
     */
    public String getACQCNTRY2() {
        return _ACQCNTRY2;
    }
    public void setACQCNTRY2(String value) {
        _ACQCNTRY2 = value;
    }
    ...
    /**
     ...
     <remark></remark>
     */
    public String getATMCUR() {
        return _ATMCUR;
    }
    public void setATMCUR(String value) {
        _ATMCUR = value;
    }
    ...
    public void setATMAMT(String value) {
        _ATMAMT = value;
    }
    ...
    /**
     ...
     <remark></remark>
     */
    public String getBASCUR() {
        return _BASCUR;
    }
    public void setBASCUR(String value) {
        _BASCUR = value;
    }
    ...
    public String makeMessageFromGeneral(MOGeneral general) throws Exception {
        MOGeneralRequest tempVar = general.getRequest();
        _TRMNO = StringUtil.toHex(StringUtils.leftPad(tempVar.getTRMNO(),5,'0'));
        ...
        _TXCD = StringUtil.toHex(StringUtils.rightPad(tempVar.getTXCD(),3,' '));
        ...
        _FXFLAG = StringUtil.toHex(StringUtils.leftPad(tempVar.getFXFLAG(),2,'0'));
        ...
        _PBAMT = this.toHex(tempVar.getPBAMT(), 13, 2); //PBAMT
        _PACTNO = StringUtil.toHex(StringUtils.leftPad(tempVar.getPACTNO(),14,'0')); //PACTNO
        _PFG = StringUtil.toHex(StringUtils.leftPad(tempVar.getPFG(),1,'0')); //PFG
        _BOAMT1 = this.toHex(tempVar.getBOAMT1(), 13, 2); //BOAMT1
        _OCURCD = StringUtil.toHex(StringUtils.leftPad(tempVar.getOCURCD(),2,'0')); //OCURCD
        _OCAMT = this.toHex(tempVar.getOCAMT(), 13, 2); //OCAMT
        _OXAMT = this.toHex(tempVar.getOXAMT(), 13, 2); //OXAMT
        _OORATE = this.toHex(tempVar.getOORATE(), 10, 5); //OORATE
        _OCRATE = this.toHex(tempVar.getOCRATE(), 10, 5); //OCRATE
        _OPBAMT = this.toHex(tempVar.getOPBAMT(), 13, 2); //OPBAMT
        ...
        _CKNO = StringUtil.toHex(StringUtils.rightPad(tempVar.getCKNO(),10,' ')); //票號
        ...
        _TRDATE = StringUtil.toHex(StringUtils.leftPad(tempVar.getTRDATE(),8,'0')); //TRANSKEY
        _TRBRNO = StringUtil.toHex(StringUtils.leftPad(tempVar.getTRBRNO(),3,'0')); //輸入行代號
        _TRWSNO = StringUtil.toHex(StringUtils.leftPad(tempVar.getTRWSNO(),2,'0')); //櫃檯機代號
        ...
        _ADDCHGE = StringUtil.toHex(StringUtils.leftPad(tempVar.getADDCHGE(),1,'0')); //ADDITIONALCHARGE(非HK地)
        _CUSFREE = this.toHex(tempVar.getCUSFREE(), 4, 2); //ATM客戶減免手續費
        _ACQCNTRY2 = StringUtil.toHex(StringUtils.rightPad(tempVar.getACQCNTRY2(),2,' ')); //交易所在地
        ...
        _FILLER2 = StringUtil.toHex(StringUtils.leftPad("", 260, ' '));

        return _TRMNO + _TXTNO + _TTSKID + _TRMTYP + _TXCD + _PTYPE + _DSCPT1 + _HCODE + _YCODE + _ACTNO + _TXTYPE + _CRDB + _SPCD + _NBCD + _TLRNO + _TRNMOD + _CURCD + _TXAMT + _FXABRN + _FXWSNO + _FXTXTNO + _FXFLAG + _CTACTNO + _SEQNO + _BDTXD + _FPBAMT + _FACTNO + _FFG + _FOAMT1 + _BCAMT + _BXAMT + _BORATE + _BCRATE + _CASH + _BKCUR + _BKSRNO + _PBAMT + _PACTNO + _PFG + _BOAMT1 + _OCURCD + _OCAMT + _OXAMT + _OORATE + _OCRATE + _OPBAMT + _OACTNO + _OFG + _OOAMT1 + _OTXCD + _ODSCPT + _CKNO + _RBRNO + _STANBKNO + _STANSSN + _SREMARK + _A_SEACUR + _A_SEAAMT + _SECNO + _HCAMT + _CUSPAY + _JTBSDY + _TPNFG + _MERRCD + _MODE + _TRDATE + _TRBRNO + _TRWSNO + _TRTXTNO + _ATMID + _PYMHC + _ADDCHGE + _CUSFREE + _ACQCNTRY2 + _FILLER1 + _LOCBRNO + _A_PLUSCUR + _A_PLUSAMT + _ATMCUR + _ATMAMT + _BASCUR + _A_BCUSPAY + _A_BCUSPRAT + _FILLER2;
    }

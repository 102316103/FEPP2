...
public class BatchService extends BaseService {
    ...
    public PageInfo<HashMap<String, Object>> getAllBatch(String batno, List<String> subsysList, int pageNum, int pageSize) throws Exception {
        try {
            // 分頁查詢
            PageInfo<HashMap<String, Object>> pageInfo = PageHelper.startPage(pageNum, pageSize, pageNum > 0 && pageSize > 0).doSelectPageInfo(new ISelect() {
                ...
                public void doSelect() {
                    batchExtMapper.getAllBatch(batno, subsysList);
                }
            });
            return pageInfo;
        } catch (Exception e) {
     ...
     * @throws Exception
     */
    public int insertBatchJobTask(Batch batch, List<Task> tasks) throws Exception {
        PlatformTransactionManager transactionManager = SpringBeanFactoryUtil.getBean(DataSourceConstant.BEAN_NAME_TRANSACTION_MANAGER);
        TransactionStatus txStatus = transactionManager.getTransaction(new DefaultTransactionDefinition());
        try {

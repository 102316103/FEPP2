...
public class IVRController extends BaseController {
    ...
    @RequestMapping(value = "/recv/ivr", method = RequestMethod.POST, produces = MediaType.APPLICATION_XML_VALUE)
    @ResponseBody
    ...
    protected String processRequestData(final ProgramFlow programFlow, final String messageIn) {
        ...
        logData.setChannel(FEPChannel.IVR);
        ...
        try {
            ...
            ivrHandler.setMessageId(logData.getMessageId());
            ivrHandler.setAtmSeq(logData.getAtmSeq());
            ...
            logData.setRemark(this.getName() + " Get Response from IVRHandler");
            logData.setChannel(FEPChannel.IVR);
            this.logMessage(logData);
        } catch (Exception e) {
            logData.setMessage(messageIn);

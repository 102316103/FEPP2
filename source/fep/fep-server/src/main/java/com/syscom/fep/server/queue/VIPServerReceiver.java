...
import com.syscom.fep.jms.queue.VIPQueueConsumers;
import com.syscom.fep.server.common.TxHelper;
import com.syscom.fep.server.common.handler.VIPHandler;
...
public class VIPServerReceiver extends FEPBase implements JmsReceiver<String> {

    @PostConstruct
    public void initialization() {
        JmsMonitorController.addNotifier(ProgramName, SpringBeanFactoryUtil.registerBean(VIPQueueConsumers.class).subscribe(this));
    }
    @Override
    public void messageReceived(String destination, String payload, Message message) {
        ...
        try {
            ...
            logData.setRemark("VIPService Receive Request");
            logData.setServiceUrl("/vip/recv");
    ...
    private String MultiProcess(String messageIn) {
        LogMDC.put(Const.MDC_PROFILE, SvrConst.SVR_VIP);
        ...
        try {
            ...
            VIPHandler vipHandler = new VIPHandler();
            vipHandler.setEj(logData.getEj());
            vipHandler.setLogContext(logData);
            ...
            messageOut = vipHandler.dispatch(channel, messageIn);
            logData.setProgramName(StringUtils.join(ProgramName, ".sendReceive"));
            logData.setProgramFlowType(ProgramFlow.RESTFulOut);
            logData.setMessageFlowType(MessageFlow.Response);
            logData.setMessage(messageOut);
            logData.setRemark("VIPService Get Response from VIPHandler");
            this.logMessage(logData);
            return messageOut;
        } catch (Exception e) {
            logData.setMessage(messageIn);
    ...
    private boolean FeeBackSuccess(String returnStr, Message messageIn) {
        try {
            ...
            sender.sendQueue(configuration.getQueueNames().getVipAck().getDestination(), returnStr, null, new JmsHandler() {
                @Override

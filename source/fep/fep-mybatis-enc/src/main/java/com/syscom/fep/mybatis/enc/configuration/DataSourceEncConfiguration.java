package com.syscom.fep.mybatis.enc.configuration;
...
@Configuration
@MapperScan(basePackages = {"com.syscom.fep.mybatis.enc.mapper", "com.syscom.fep.mybatis.enc.ext.mapper"}, annotationClass = Resource.class, sqlSessionTemplateRef = DataSourceEncConstant.BEAN_NAME_SQL_SESSION_TEMPLATE)
@RefreshScope
public class DataSourceEncConfiguration implements DataSourceEncConstant {
    // @Value("${spring.fep.mybatis.enc.interceptor.connection.detected.enable:false}")
    ...
    public DataSource dataSource(@Qualifier(BEAN_NAME_HIKARI_CONFIG) HikariConfig hikariConfig) throws Exception {
        ...
        File acctFile = pkiConfig.getPkiAcctFile(FEPDBName.DESDB);
        LogHelperFactory.getGeneralLogger().info(" enc acdu Path : "+acctFile);
        File sscodeFile = pkiConfig.getPkiSscodeFile(FEPDBName.DESDB);
        LogHelperFactory.getGeneralLogger().info(" enc acdp Path : "+sscodeFile);
        try (InputStream acctIn = IOUtil.openInputStream(acctFile.getPath());
            ...
            LogHelperFactory.getGeneralLogger().info(" enc username  : "+acctProp.getProperty(CONFIGURATION_PROPERTIES_USERNAME));
            LogHelperFactory.getGeneralLogger().info(" enc password  : ***"+Jasypt.decrypt(sscodeProp.getProperty(CONFIGURATION_PROPERTIES_PASSWORD)));
...
//    }
    ...
    public SqlSessionFactory testSqlSessionFactory(@Qualifier(BEAN_NAME_DATASOURCE) DataSource dataSource) throws Exception {
        ...
        bean.setMapperLocations(
                ArrayUtils.addAll(
                        new PathMatchingResourcePatternResolver().getResources("classpath*:com/syscom/fep/mybatis/enc/mapper/xml/*.xml"),
                        new PathMatchingResourcePatternResolver().getResources("classpath*:com/syscom/fep/mybatis/enc/ext/mapper/xml/*.xml")));
        bean.setConfigLocation(new PathMatchingResourcePatternResolver().getResource("classpath:mybatis-config.xml"));
        // 增加攔截器
        List<Interceptor> interceptorList = new ArrayList<Interceptor>();
//        if (dataSource instanceof DynamicDataSource && this.connectionDetectedEnable) {
//            EncConnectionDetectedInterceptor encConnectionDetectedInterceptor = new EncConnectionDetectedInterceptor();
//            interceptorList.add(encConnectionDetectedInterceptor);
//        }
    ...
    public void print() {
        printConstant();
        Field[] fields = DataSourceEncConfiguration.class.getDeclaredFields();

...
public class SampleGatewayClientProcessRequest extends NettyTransmissionChannelProcessRequestClient<SampleGatewayClientConfiguration> {
    ...
    public void doProcess(ChannelHandlerContext ctx, byte[] bytes) throws Exception {
        ...
        this.logContext.setRemark(StringUtils.join(Gateway.SAMPLEGW, " received message from Server"));
        ...
        try {
            ...
            this.logContext.setMessage(message);
            this.logContext.setProgramName(StringUtils.join(ProgramName, ".doProcess"));
            this.logContext.setRemark(StringUtils.join(Gateway.SAMPLEGW, " send message to Server"));
            this.logMessage(this.logContext);
        } catch (Exception e) {

...
public class FN000411 extends ENCFunction {
	/**
	 * 這個構建函數一定要增加
	 * 
	 * @param suipData
	 */
	public FN000411(SuipData suipData) {
		super(suipData);
	}
	...
	public SuipData process() throws Exception {
		...
		// 4.拆解Suip回傳結果
		rc = this.parseFN000411ReturnData(rtn);
		return this.suipData;
	}

	private ENCRC checkFN000411Data() {
		// Key:MAC ATM ATM_ID
	...
	private ENCRC parseFN000411ReturnData(String suipReturn) throws DecoderException {
		...
		if (suipReturn.length() > 47) {
			...
			this.suipData.setRc(Integer.parseInt(suipRc, 16)); // 先將Suip回應的RC塞至this.suipData中
			if (this.suipData.getRc() == 0) {
				// Return_data_1=”0008”+ MAC_DATA(N8)

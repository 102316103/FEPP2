...
public class FN000501 extends ENCFunction {
	/**
	 * 這個構建函數一定要增加
	 * 
	 * @param suipData
	 */
	public FN000501(SuipData suipData) {
		super(suipData);
	}
	...
	public SuipData process() throws Exception {
		...
		// 3.Call suip取得回應資料
		String schemaID = this.suipData.getInputData1().substring(4, 5);
		String inputData = StringUtils.join(this.suipData.getInputData1().substring(5), this.suipData.getInputData2());
		String command = this.getSuipCommand(fn, schemaID, keyData.getKeys()[0].KeyType, key, inputData);
		...
		// 4.拆解Suip回傳結果
		rc = this.parseFN000501ReturnData(rtn);
		return this.suipData;
	}

	private ENCRC checkFN000501Data() {
	...
	private ENCRC parseFN000501ReturnData(String suipReturn) throws DecoderException {
		ENCRC rc = ENCRC.SuipReturnError;
		// Return Data 12 byte header + 4 byte rc + 8 byte aprc=24

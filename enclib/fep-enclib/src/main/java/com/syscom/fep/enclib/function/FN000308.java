...
public class FN000308 extends ENCFunction {
	/**
	 * 這個構建函數一定要增加
	 * 
	 * @param suipData
	 */
	public FN000308(SuipData suipData) {
		super(suipData);
	}
	...
	public SuipData process() throws Exception {
		...
		rc = this.checkFN000308Data(refSuip);
		...
		// 3.Call suip取得回應資料
		String inputData = StringUtils.join(suip[0].getInputData1(), suip[1].getInputData1(), suip[1].getInputData2());
		...
		// 4.拆解Suip回傳結果
		rc = this.parseFN000308ReturnData(rtn);
		return this.suipData;
	}

	private ENCRC checkFN000308Data(RefBase<SuipData[]> suip) {
	...
	private ENCRC parseFN000308ReturnData(String suipReturn) {
		ENCRC rc = ENCRC.SuipReturnError;
		// 301 Return Data 12byte header + 4byte rc=16
		if (suipReturn.length() > 31) {

<#--
修正記錄：
2023/06/10，使用FreeMarker摸板引擎，NB
...
public class ${xlsxFile.className} extends NbXmlBase {
    <#-- 處理屬性宣告 -->
    @XStreamAsAttribute
    ...
    public static class ${xlsxFile.className}_Body_MsgRq_SvcRq {
        ...
        private ${xlsxFile.className}_Body_MsgRq_SvcRq_PayData paydata;
        <#--  基本屬性，getter、setter -->
        public ${xlsxFile.className}_Body_MsgRq_SvcRq_PayData getPaydata() {
            return paydata;
        }
        public void setPaydata(${xlsxFile.className}_Body_MsgRq_SvcRq_PayData paydata) {
    ...
    <#-- 處理PAYDATA屬性 -->
    @XStreamAlias("PAYDATA")
    public static class ${xlsxFile.className}_Body_MsgRq_SvcRq_PayData {
        <#-- 定制屬性 -->
        <#list xlsxFile.xlsxList as xlsx>
        <#if "PAYDATA" == xlsx.headerBody?upper_case>
        private String ${xlsx.name};
        </#if>
        </#list>

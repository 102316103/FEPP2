 ...
 * 注意：excel檔案第一欄內容須填寫 HEADER 或 BODY 或 SENDDATA 或 REPLYDATA
 *  	而 SENDDATA、REPLYDATA 標千則須放空白 ； SENDDATA=上行，REPLYDATA=下行
 */
public class ReadFile4VA extends ReadSingleFile {
	@Override
	public void doWrite(XlsxFile xlsxFile) throws IOException {
		...
		try {
			...
			data.put("package", "com.syscom.fep.vo.text.nb"); // 定義java檔所在包
			...
			if (StringUtils.equalsIgnoreCase("request", xlsxFile.getCellMessageType())) {
				data.put("headerTypeList", Arrays.asList( // 設置電文說明的父子類關系
						...
						new HeaderType("BODY", "SvcRq").addSubHeaderList(new HeaderType("SENDDATA", "SENDDATA")), //
						new HeaderType("SENDDATA", "SENDDATA")));
			} else {
				data.put("headerTypeList", Arrays.asList( // 設置電文說明的父子類關系
						new HeaderType("HEADER", "Header"), //
						new HeaderType("BODY", "SvcRs").addSubHeaderList(new HeaderType("REPLYDATA", "REPLYDATA")), //
						new HeaderType("REPLYDATA", "REPLYDATA")));
			}

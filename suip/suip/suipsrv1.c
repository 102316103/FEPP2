...
void main( int argc, char* argv[] )
{
   ...
   HSMLogMSG( "hsmsuip;", "hsmsuip start,version=1.0.1.10", NULL ) ;
...
int SVconIpaddrCHK( pSTRipPORT_t xp, pCtxtKEY_t pCntx, unsigned long ipadr )
{
   ...
   if ( i >= MaxCONN ) return( -90 ) ; /* reject connection */
...
static int SVRportPRM( void )
{
   ...
   /* Jim, 2012/5/23, modify port=8002 */ 
   (ip+n++)->port = (cport = 8002) ;
   return( n ) ;
}

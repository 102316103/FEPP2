...
/*-------------------------------------------------------------------------------*/
/*  FUNCTION CODE FN000301 : 產生MAC DATA欄位資料                                */
/*-------------------------------------------------------------------------------*/
/*
...
{
   int  n, mklen ;
   char cmds[4096], mackey[50], icv[16] ; 
   pfn301i_t i301 = (pfn301i_t) in ;
   pfn301o_t o301 = (pfn301o_t) out ;
   ...
   if( i301->keytype == 0x01 )
       memcpy( mackey, i301->key, 16 ) ;
   else
   if( i301->keytype == 0x02 ){
       memcpy( mackey, "U", 1 ) ;
       memcpy( mackey+1, i301->key, 32 ) ;
   }
   else
   if( i301->keytype == 0x03 ){
       memcpy( mackey, "T", 1 ) ;
 	   memcpy( mackey+1, i301->key, 48 ) ;
   }
   else
	   return( o301->rc = 04 ) ;

   mklen = strlen( mackey ) ;
   n = ldrint( i301->datalen, sizeof(i301->datalen) ) ;   

   memset( icv, '0', 16 ) ;
   memcpy( icv, i301->data, 6 ) ;

   strcpy( cmds, "SE" ) ;
   memcpy( cmds+2, "14", 2 ) ;
   memcpy( cmds+4, mackey, mklen ) ;
   memcpy( cmds+4+mklen, icv, 16 ) ;
   memcpy( cmds+20+mklen, i301->data+6, n-6 );

   HSMLogMSG( "FN000301;", cmds, NULL ) ;
   if( o301->rc = hsmio(cmds, 0) ){
       HSMLogMSG( "FN000301;", cmds, NULL ) ;
       return( o301->rc ) ;
   }
   memcpy( o301->mac, sf->mab, 16 ) ;
   HSMLogMSG( "FN000301;", cmds, NULL ) ;
   return( o301->rc ) ;
}
